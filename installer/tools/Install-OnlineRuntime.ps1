...
param(
    ...
    [string]$CacheRoot = (Join-Path $env:TEMP 'FR-SAM-Text-Selection-0.9.0'),
...
if (
    ...
    # Reuse a complete runtime if a later model/config/preflight stage failed.
    Move-Item -LiteralPath $runtime -Destination $runtimeStage
}
...
if (-not $torchReady) {
    ...
    Invoke-Pip $python $pipWheel @(
        'install', '--disable-pip-version-check', '--only-binary=:all:',
...
Invoke-Pip $python $pipWheel @(
    'install', '--disable-pip-version-check', '--no-cache-dir', '--only-binary=:all:',
...
if ($ModelMode -eq 'download') {
    ...
    [IO.File]::WriteAllText($downloadMarker, 'downloaded-by=0.9.0' + [Environment]::NewLine, [Text.UTF8Encoding]::new($false))
} else {
...
$config = @(
    '# Generated by FR SAM Text Selection 0.9.0 installer'
    'schemaVersion=2'
...
[IO.File]::WriteAllText($configPath, $config + [Environment]::NewLine, [Text.UTF8Encoding]::new($false))
