...
从插件 0.9.3 起，经过请求 ID 校验的 `NO_OBJECT` 错误被 Photoshop 端转换为“全画布选区并继续”，无需输出掩码文件，也不重试。后端错误语义不变；服务仍返回 `status: error` 与 `code: NO_OBJECT`，不伪造成功检测或评分。该后备同时应用于 UXP 和 ExtendScript，明确覆盖旧 ROI；成功掩码仍严格遵循 ROI。V1 服务错误响应可不带 schemaVersion，若显式提供则必须为 1。其他错误不得通过文字匹配转换为全选。
